...
export class KurentoService {
  ...
  private uiRemoteVideo: HTMLVideoElement;
  private uiLocalVideo: HTMLVideoElement;

  constructor() {
    this.uiSetState(this.UI_IDLE);
  }

  start(uiLocalVideo: HTMLVideoElement, uiRemoteVideo: HTMLVideoElement): void {
    this.uiLocalVideo = uiLocalVideo;
    this.uiRemoteVideo = uiRemoteVideo;
  ...
  handleMessage(message: any): void {
    switch (message.id) {
      ...
      case 'ERROR':
        ...
        break;
      default:
        console.warn('[handleMessage] Invalid message, id: ' + message.id);
        break;
    }
  }

  handleProcessSdpAnswer(message: any): void {
    ...
    this.webRtcPeer.processAnswer(message.sdpAnswer, (err) => {
      if (err) {
  ...
  uiStart(): void {
    ...
    const options = {
      localVideo: this.uiLocalVideo,
      remoteVideo: this.uiRemoteVideo,
...
// UTILS ______________________________________________________
